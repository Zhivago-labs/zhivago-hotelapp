...
export default function CreateListingScreen() {
  ...
  const handleSave = async (status: 'DRAFT' | 'APPROVED') => {
    if (!name || !price || imageUris.length === 0 || !cep) {
    ...
    for (let i = 0; i < imageUris.length; i++) {
      ...
      if (Platform.OS === 'web') {
        try {
          ...
          formData.append('images', blob, filename || `upload-${i}.jpg`);
        } catch (e) {
          Alert.alert('Erro', 'Falha ao processar uma das imagens no navegador.');
          setIsSaving(false);
          return;
        }
      } else {
    ...
    try {
      ...
      if (response.ok) {
        Alert.alert(
          'Sucesso',
          status === 'DRAFT'
            ? 'Rascunho salvo! Você pode publicá-lo quando estiver pronto, em "Meus Imóveis".'
  ...
  if (user?.role === 'ADMIN') {
    return (
      <View style={[styles.container, { flex: 1, justifyContent: 'center', alignItems: 'center', padding: 24 }]}>
        <Text style={{ fontSize: 16, color: '#666', textAlign: 'center' }}>
          Administradores não podem anunciar imóveis.
        </Text>
      </View>
    );
  }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 40 }}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.imageRow}>
        ...
        <TouchableOpacity style={styles.imagePicker} onPress={pickImage}>
          <View style={styles.imagePlaceholder}>
            ...
            <Text style={{ color: '#ccc', fontSize: 11 }}>Adicionar</Text>
          </View>
        </TouchableOpacity>
      </ScrollView>
      ...
      <TouchableOpacity
        style={[styles.draftButton, isSaving && { opacity: 0.7 }]}
        onPress={() => handleSave('DRAFT')}
        disabled={isSaving}
      >
        <Text style={styles.draftButtonText}>Salvar como Rascunho</Text>
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  imagePicker: { width: 100, height: 100, borderRadius: 10, borderStyle: 'dashed', borderWidth: 1, borderColor: '#ccc', overflow: 'hidden' },
  imagePlaceholder: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  imageThumbWrap: { width: 100, height: 100, borderRadius: 10, overflow: 'hidden', marginRight: 10, position: 'relative' },
  imageThumb: { width: '100%', height: '100%' },
  imageRemoveButton: {
    position: 'absolute', top: 4, right: 4, width: 20, height: 20, borderRadius: 10,
    backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center',
  },
  draftButton: { borderWidth: 1.5, borderColor: '#ccc', padding: 15, borderRadius: 10, alignItems: 'center', marginTop: 20 },
  draftButtonText: { color: '#666', fontSize: 15, fontWeight: 'bold' },
  saveButton: { backgroundColor: '#ff385c', padding: 15, borderRadius: 10, alignItems: 'center', marginTop: 12 },
