...
export default function MyListingsScreen() {
  ...
  const submitStatusChange = async (id: string, status: 'DRAFT' | 'APPROVED') => {
    try {
      const res = await fetch(`${API_URL}/listings/${id}`, {
        method: 'PUT',
  ...
  const handleDuplicate = async (id: string) => {
    try {
      const res = await fetch(`${API_URL}/listings/${id}/duplicate`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
        ...
        loadMyListings();
        router.push(`/editar-anuncio/${duplicate.id}` as never);
      } else {
        Alert.alert('Erro', 'Não foi possível duplicar o anúncio.');
      }
    } catch {
      Alert.alert('Erro de conexão', 'Verifique o servidor.');
    }
        ...
        <View style={styles.cardBody}>
          ...
          <View style={styles.metricsRow}>
            <View style={styles.metricItem}>
              ...
              <Text style={styles.metricText}>{item.viewCount} visualizações</Text>
            </View>
            <View style={styles.metricItem}>
              <Ionicons name="chatbubble-outline" size={14} color="#94a3b8" />
              <Text style={styles.metricText}>{item._count.conversations} contatos</Text>
            </View>
          </View>
            ...
            <View style={styles.actions}>
              ...
              {item.status === 'DRAFT' && (
                <TouchableOpacity
                  ...
                  onPress={() => submitStatusChange(item.id, 'APPROVED')}
                >
              ...
              {item.status === 'APPROVED' && (
                <TouchableOpacity
                  ...
                  onPress={() => submitStatusChange(item.id, 'DRAFT')}
                >
...
const styles = StyleSheet.create({
  ...
  metricText: { fontSize: 12, color: '#94a3b8', fontWeight: '600' },
  ...
  unpublishBtn: {
    borderWidth: 1.5, borderColor: '#64748b',
    padding: 6, borderRadius: 20,
  },
  duplicateBtn: {
