...
type Booking = {
  ...
  endDate: string;
  status: string;
  // Preço efetivo travado no momento da reserva — nunca o preço ao vivo do anúncio.
  price?: number;
  listing: {
    id: string;
    name: string;
...
export default function MinhasViagensScreen() {
  ...
  const renderBooking = ({ item }: { item: Booking }) => {
    ...
    const unitPrice = item.price ?? item.listing.price;
        ...
        <View style={styles.cardInfo}>
          ...
          <View style={styles.bottomRow}>
            <Text style={styles.priceText}>
              R$ {unitPrice.toLocaleString('pt-BR')} <Text style={{ fontSize: 12, fontWeight: 'normal', color: '#666' }}>/ noite</Text>
            </Text>
