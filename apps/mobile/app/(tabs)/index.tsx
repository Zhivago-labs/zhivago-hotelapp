...
export default function TabOneScreen() {
  ...
  const filteredData = React.useMemo(() => {
    ...
    let data = allListings.filter((item) => {
      ...
      const itemBedrooms = item.amenities?.bedrooms || item.bedrooms || 0;
      const itemBathrooms = item.amenities?.bathrooms || item.bathrooms || 0;
      const itemParking = item.amenities?.parking || item.parking || 0;

      const matchesBedrooms = minBedrooms === null || itemBedrooms >= minBedrooms;
      const matchesBathrooms = minBathrooms === null || itemBathrooms >= minBathrooms;
      const matchesParking = minParking === null || itemParking >= minParking;

      const matchesMinPrice = minP === null || item.price >= minP;
      const matchesMaxPrice = maxP === null || item.price <= maxP;

      return matchesType && matchesCategory && matchesBedrooms && matchesBathrooms && matchesParking && matchesMinPrice && matchesMaxPrice && (
    ...
    return data;
  }, [allListings, activeType, activeCategory, searchQuery, sortOrder, minBedrooms, minBathrooms, minParking, minPrice, maxPrice]);
              ...
              <ScrollView>
                ...
                <View style={{ flexDirection: 'row', gap: 10, marginBottom: 16 }}>
                  <TextInput
                    ...
                    style={{ flex: 1, backgroundColor: '#f1f5f9', borderRadius: 10, padding: 12, fontSize: 14, color: '#0f172a' }}
                  />
                  <TextInput
                    placeholder="Máximo"
                    keyboardType="numeric"
                ...
                <Text style={styles.sectionTitle}>Mínimo de Quartos</Text>
                <View style={styles.optionRow}>
                  {[1, 2, 3, 4].map((num) => (
                    <TouchableOpacity
                      key={num}
                      style={[styles.optionCard, minBedrooms === num && styles.optionCardActive]}
                      onPress={() => setMinBedrooms(minBedrooms === num ? null : num)}
                    >
                      <Text style={[styles.optionText, minBedrooms === num && styles.optionTextActive]}>{num}+</Text>
                    </TouchableOpacity>
                  ))}
                </View>

                <Text style={styles.sectionTitle}>Mínimo de Banheiros</Text>
                <View style={styles.optionRow}>
                  {[1, 2, 3].map((num) => (
                    <TouchableOpacity
                      key={num}
                      style={[styles.optionCard, minBathrooms === num && styles.optionCardActive]}
                      onPress={() => setMinBathrooms(minBathrooms === num ? null : num)}
                    >
                      <Text style={[styles.optionText, minBathrooms === num && styles.optionTextActive]}>{num}+</Text>
                    </TouchableOpacity>
                  ))}
                </View>

                <Text style={styles.sectionTitle}>Mínimo de Vagas</Text>
                <View style={styles.optionRow}>
                  {[1, 2, 3].map((num) => (
                    <TouchableOpacity
                      key={num}
                      style={[styles.optionCard, minParking === num && styles.optionCardActive]}
                      onPress={() => setMinParking(minParking === num ? null : num)}
                    >
                      <Text style={[styles.optionText, minParking === num && styles.optionTextActive]}>{num}+</Text>
                    </TouchableOpacity>
                  ))}
                </View>
