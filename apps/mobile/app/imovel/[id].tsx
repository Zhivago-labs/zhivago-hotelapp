...
import { View, Text, StyleSheet, ScrollView, ActivityIndicator, TouchableOpacity, Alert, Platform, Linking, TextInput, Modal, useWindowDimensions } from 'react-native';
...
export default function ListingDetailScreen() {
  ...
  const handleInAppChat = async () => {
    ...
    if (listing?.category === 'aluguel') {
      if (user?.role === 'ADMIN') {
        Alert.alert('Acesso restrito', 'Administradores não podem fazer reservas.');
        return;
      }
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.favoriteHeaderButton} onPress={() => toggleFavorite(id || '')}>
        <Ionicons name={isFav ? 'heart' : 'heart-outline'} size={24} color={isFav ? '#ff385c' : '#fff'} />
      </TouchableOpacity>
          ...
          {listing.images.length > 1 && (
            <View style={styles.dotsRow}>
              {listing.images.map((img, i) => (
                <View key={img.id} style={[styles.dot, i === activeImageIndex && styles.dotActive]} />
              ))}
            </View>
          )}
        </View>
...
const styles = StyleSheet.create({
  ...
  favoriteHeaderButton: { position: 'absolute', top: 50, right: 20, zIndex: 10, backgroundColor: 'rgba(0,0,0,0.4)', padding: 8, borderRadius: 20 },
