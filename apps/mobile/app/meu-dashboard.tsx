...
export default function UserDashboardScreen() {
  ...
  const handleExportCSV = () => {
    if (!bookingsData.bookings || bookingsData.bookings.length === 0) {
      Alert.alert('Aviso', 'Nenhuma reserva disponível para exportação.');
      return;
    }
    ...
    const rows = bookingsData.bookings.map((b) => [
      ...
      b.startDate ? new Date(b.startDate).toLocaleDateString('pt-BR') : '',
      b.endDate ? new Date(b.endDate).toLocaleDateString('pt-BR') : '',
      b.status === 'CONFIRMED' || b.status === 'APPROVED' ? 'Confirmada' : b.status === 'PENDING' ? 'Pendente' : b.status,
    ...
    Alert.alert(
      '📊 Relatório CSV de Reservas',
      `Foram encontradas ${bookingsData.bookings.length} reserva(s):\n\n${csvText.slice(0, 350)}...`,
      [{ text: 'OK' }]
    );
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 16, paddingBottom: 100 }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 24, marginBottom: 12 }}>
        <Text style={[styles.chartTitle, { marginTop: 0, marginBottom: 0 }]}>Reservas Recebidas</Text>
        <TouchableOpacity
          onPress={handleExportCSV}
          style={{ flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#fff', borderWidth: 1, borderColor: '#e2e8f0', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8 }}
        >
          <Ionicons name="download-outline" size={16} color="#ff385c" />
          <Text style={{ fontSize: 13, fontWeight: '700', color: '#ff385c' }}>Exportar CSV</Text>
        </TouchableOpacity>
      </View>
